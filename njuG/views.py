...
def profile(request):
	if (request.method == "GET"):
		...
		profileForm = ProfileForm(
			initial={'nickName': profile.nickName, 'school': profile.school, 'birth_year': profile.birth_year,
					 'role': profile.role})
...
def myDiscussion(request, **kwargs):
	if (request.method == 'GET'):
		if ('id' in kwargs):
			...
			user = User.objects.get(pk=id)
			target = user
			blog_list = Blog.objects.filter(user=target)
		else:
			target = request.user
			blog_list = Blog.objects.filter(user=target)
...
def postActivity(request):
	if (request.method == 'POST'):
		...
		if (form.is_valid()):
			...
			openSchoolListData = u' '.join(openSchoolList)
